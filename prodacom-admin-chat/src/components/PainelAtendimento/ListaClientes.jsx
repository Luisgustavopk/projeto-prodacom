...
import { Monitor, ChevronDown, Trash2, Ban, MessageCircle } from "lucide-react";
...
function BadgeStatus({ status }) {
  ...
  if (status === 'em_andamento') return <span className={`${baseClasses} bg-blue-500/10 text-blue-400 border-blue-500/20`}>Em Atend.</span>;
  if (status === 'finalizado') return <span className={`${baseClasses} bg-emerald-500/10 text-emerald-400 border-emerald-500/20`}>Finalizado</span>;
  return <span className={`${baseClasses} bg-amber-500/10 text-amber-500 border-amber-500/20`}>Aberto</span>;
}

export function ListaClientes({ chats, clienteAtivo, setClienteAtivo, canalAtivo, setCanalAtivo }) {
  const [menuAbertoId, setMenuAbertoId] = useState(null);

  const conversasOrdenadas = Object.entries(chats)
    .filter(([_, dados]) => (dados.canal || 'site') === canalAtivo)
    .sort((a, b) => {
  ...
  return (
    <div className={`${clienteAtivo ? "hidden md:flex" : "flex"} w-full md:w-1/3 md:max-w-sm bg-[#1a1a1a] flex-col border-r border-white/5 shadow-2xl z-20`} onClick={() => setMenuAbertoId(null)}>
      ...
      <div className="p-4 md:p-6 bg-[#1a1a1a] border-b border-white/5 flex flex-col gap-4 shrink-0">
        ...
        <div className="flex gap-2 w-full mt-2">
          <button 
            onClick={() => { setCanalAtivo('site'); setClienteAtivo(null); }}
            className={`flex-1 flex justify-center items-center gap-2 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${canalAtivo === 'site' ? 'bg-[#2563eb] text-white shadow-lg' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
          >
            <Monitor size={12} /> Site
          </button>
          <button 
            onClick={() => { setCanalAtivo('whatsapp'); setClienteAtivo(null); }}
            className={`flex-1 flex justify-center items-center gap-2 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${canalAtivo === 'whatsapp' ? 'bg-emerald-600 text-white shadow-lg' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
          >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-16">
        {conversasOrdenadas.length === 0 ? (
          <div className="text-white/20 text-center text-[10px] uppercase tracking-widest p-10 mt-10 font-medium">Nenhum chat no {canalAtivo}</div>
        ) : (
            ...
            return (
              <div key={id} className="relative group">
                <button
                  ...
                  className={`w-full text-left p-4 md:p-5 border-b border-white/5 transition-all flex flex-col gap-2.5 ${isSelected ? (canalAtivo === 'whatsapp' ? 'bg-emerald-900/40' : 'bg-sky-800') : "hover:bg-white/5"}`}
                >
                  <div className="flex justify-between items-start w-full pr-2">
                    <div className="flex items-center gap-2.5 overflow-hidden">
                      {canalAtivo === 'site' && <span className={`w-2 h-2 shrink-0 rounded-full transition-all duration-300 ${dados.online ? "bg-emerald-400 animate-pulse" : "bg-slate-500"}`} />}
