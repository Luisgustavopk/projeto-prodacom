...
export function ConversaAtiva({ clienteAtivo, setClienteAtivo, chat, input, setInput, handleSend, mudarStatusAtendimento }) {
  ...
  return (
    <div className="flex flex-1 flex-col bg-slate-50 z-10 w-full relative" onClick={() => { setMenuMsgId(null); setMenuStatusAberto(false); }}>
      <div className="px-4 md:px-8 py-4 md:py-5 bg-[#1a1a1a] border-b border-white/10 flex items-center justify-between shadow-md shrink-0">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="flex w-10 h-10 md:w-12 md:h-12 bg-[#2563eb] text-white items-center justify-center shadow-lg rounded-full"><User size={20} strokeWidth={1.5} /></div>
          <div>
        ...
        <div className="relative shrink-0" onClick={(e) => e.stopPropagation()}>
          ...
          {menuStatusAberto && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-[#2a2a2a] border border-white/10 shadow-2xl rounded-xl z-[70] py-1.5 overflow-hidden">
              <button 
                onClick={(e) => { e.stopPropagation(); if(mudarStatusAtendimento) mudarStatusAtendimento(chat.contato, 'aberto'); setMenuStatusAberto(false); }} 
                className="w-full text-left px-5 py-2.5 text-[10px] font-bold tracking-widest text-amber-500 hover:bg-white/5 uppercase transition-colors"
              >Aberto</button>
              <button 
                onClick={(e) => { e.stopPropagation(); if(mudarStatusAtendimento) mudarStatusAtendimento(chat.contato, 'em_andamento'); setMenuStatusAberto(false); }} 
                className="w-full text-left px-5 py-2.5 text-[10px] font-bold tracking-widest text-blue-400 hover:bg-white/5 uppercase transition-colors"
              >Em Atendimento</button>
              <button 
                onClick={(e) => { e.stopPropagation(); if(mudarStatusAtendimento) mudarStatusAtendimento(chat.contato, 'finalizado'); setMenuStatusAberto(false); }} 
                className="w-full text-left px-5 py-2.5 text-[10px] font-bold tracking-widest text-emerald-400 hover:bg-white/5 uppercase transition-colors"
              >Finalizado</button>
            </div>
          )}
        </div>
      ...
      <div className="p-3 md:p-6 bg-white border-t border-slate-200 shrink-0" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSend} className="flex items-center gap-2 md:gap-3 max-w-5xl mx-auto">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="RESPONDER..." className="flex-1 bg-slate-50 border border-slate-200 px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-xs font-medium text-[#1a1a1a] placeholder:text-slate-400 focus:outline-none focus:border-[#2563eb] transition-all uppercase tracking-widest rounded-xl" />
          <button type="submit" className="bg-[#2563eb] text-white w-12 h-12 md:w-14 md:h-14 flex items-center justify-center hover:bg-[#1a1a1a] transition-all shadow-md active:scale-95 shrink-0 rounded-xl"><Send size={18} className="md:w-5 md:h-5" /></button>
        </form>
      </div>
    </div>
  );
