...
export function ConversaAtiva({ clienteAtivo, setClienteAtivo, chat, input, setInput, handleSend }) {
  ...
  const [menuMsgId, setMenuMsgId] = useState(null); // 👈 Estado do dropdown da mensagem
  ...
  return (
    <div className="flex flex-1 flex-col bg-slate-50 z-10 w-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 md:space-y-6 bg-slate-50 custom-scrollbar">
        {chat.mensagens?.map((msg, i) => {
          ...
          return (
            <div key={i} className={`flex group relative ${isAdmin ? "justify-end" : "justify-start"}`}>
              {!isAdmin && (
                <div className="w-6 h-6 md:w-8 md:h-8 bg-[#1a1a1a] text-white flex items-center justify-center mr-2 md:mr-3 mt-1 shadow-md shrink-0"><MessageSquare size={12} className="md:w-3.5 md:h-3.5" /></div>
              )}
              
              <div 
                onMouseLeave={() => setMenuMsgId(null)} // Fecha o menu ao sair do balão
                className="flex flex-col gap-1 max-w-[85%] md:max-w-[65%] relative"
              >
                {/* 👇 Wrapper flexível para alinhar o balão e o menu dropdown */}
                <div className="relative flex items-center gap-2">

                  {/* Dropdown Excluir (Aparece à esquerda do balão Azul e APENAS para Admin) */}
                  {isAdmin && !msg.apagada && msg.id && (
                    <div className="absolute top-1/2 -translate-y-1/2 -left-7 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                      <button 
                        onClick={() => setMenuMsgId(menuMsgId === msg.id ? null : msg.id)}
                        className="p-1 text-slate-300 hover:text-slate-500 hover:bg-slate-200 rounded-full transition-colors"
                      >
                        <ChevronDown size={14} />
                      </button>

                      {menuMsgId === msg.id && (
                        <div className="absolute right-full top-full mr-1 mt-1 w-28 bg-white border border-slate-200 shadow-xl rounded-md z-20 py-1 overflow-hidden">
                          <button 
                            onClick={() => {
                  ...
                  {/* Estilos Polidos para a Mensagem (Limpando o "emoji feio") */}
                  <div className={`px-4 md:px-5 py-2.5 md:py-3 text-xs md:text-sm leading-relaxed shadow-sm flex items-center gap-2 ${
                    msg.apagada 
                      ? "italic text-slate-400 bg-slate-100 border border-slate-200 rounded-lg" 
                      : isAdmin 
                        ? "bg-[#2563eb] text-white border border-[#2563eb]" 
                        : "bg-white border border-slate-200 text-[#1a1a1a]"
                  }`}>
                    {msg.apagada && <Ban size={14} className="text-slate-400/70 shrink-0" />}
                    {/* Renderiza tirando o emoji do backend, para ficar só a paleta */}
                    {msg.content?.replace("🚫 ", "") || msg.texto}
                  </div>
                </div>
