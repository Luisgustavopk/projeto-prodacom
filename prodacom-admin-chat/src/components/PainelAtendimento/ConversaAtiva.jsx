...
import { MessageSquare, Send, User, Phone, ChevronLeft, Check, CheckCheck } from "lucide-react";
    ...
    <div className="flex flex-1 flex-col bg-slate-50 z-10 w-full">
      ...
      <div className="px-4 md:px-8 py-4 md:py-5 bg-[#1a1a1a] border-b border-white/10 flex items-center justify-between shadow-md shrink-0">
        <div className="flex items-center gap-3 md:gap-4">
          <button onClick={() => setClienteAtivo(null)} className="md:hidden flex items-center justify-center w-10 h-10 bg-white/5 text-white hover:bg-white/10">
            <ChevronLeft size={20} />
          </button>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-white/50 uppercase font-mono">{chat.online ? "Online" : "Offline"}</span>
            </div>
            <div className="flex items-center gap-2 md:gap-4 mt-0.5 md:mt-1">
              <a href={`https://wa.me/${chat.contato?.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="text-sky-700 hover:text-white transition-colors text-[9px] md:text-[10px] font-mono font-bold flex items-center gap-1">
                <Phone size={10} /> {chat.contato}
              </a>
            </div>
          </div>
        </div>
          ...
          return (
            <div key={i} className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
              ...
              <div className="flex flex-col gap-1 max-w-[85%] md:max-w-[65%]">
                <div className={`px-4 md:px-5 py-2.5 md:py-3 text-xs md:text-sm leading-relaxed shadow-sm ${
                  isAdmin ? "bg-[#2563eb] text-white border border-[#2563eb]" : "bg-white border border-slate-200 text-[#1a1a1a]"
                }`}>
                  {msg.content || msg.texto}
                </div>
